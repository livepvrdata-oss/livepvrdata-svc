...
class EPL extends Monitor {
	static private final String SPORT = 'soccer'
	static private final String LEAGUE = 'epl'
	static String getNormalizedTitle() { 'English Premiere League Football' }
	static Event[] getEventsForDate(long date) throws IOException {	Monitor.getEventsForDate(LEAGUE, date) }

	EPL(String desc, long date) {
		super(desc, date, SPORT, LEAGUE)
	}

	EPL(String[] teams, long date) {
