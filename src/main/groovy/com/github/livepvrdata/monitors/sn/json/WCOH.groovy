...
class WCOH extends Monitor {
	static private final String SPORT = 'hockey'
	static private final String LEAGUE = 'wcoh'
	static String getNormalizedTitle() { 'World Cup of Hockey' }
	static Event[] getEventsForDate(long date) throws IOException {	Monitor.getEventsForDate(LEAGUE, date) }

	WCOH(String desc, long date) {
		super(desc, date, SPORT, LEAGUE)
	}

	WCOH(String[] teams, long date) {
