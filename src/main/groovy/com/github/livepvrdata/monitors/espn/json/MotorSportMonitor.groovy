...
abstract class MotorSportMonitor extends EventMonitor {
	static final String FEED_URL = 'http://site.api.espn.com/apis/v2/scoreboard/header?sport=%s&league=%s&disable=links,competitors,broadcasts&dates=%s'
	...
	static final Event[] getFeed(String sport, String league, String date) throws IOException {
		...
		if(events == null) {
			try {
				def data = new URL(String.format(FEED_URL, sport, league, date)).text
