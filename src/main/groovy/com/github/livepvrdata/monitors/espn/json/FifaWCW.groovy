...
class FifaWCW extends Monitor {
	static private final String SPORT = 'soccer'
	static private final String LEAGUE = 'fifa.wwc'
	static String getNormalizedTitle() { 'FIFA Women\'s World Cup' }
	static Event[] getEventsForDate(long date) throws IOException {	Monitor.getEventsForDate(SPORT, LEAGUE, date) }
	
	FifaWCW(String desc, long date) {
		super(desc, date, SPORT, LEAGUE)
	}

	FifaWCW(String[] teams, long date) {
